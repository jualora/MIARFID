...
(define (domain dominio_robots)
  ...
  (:durative-action move-robot
		 ...
		 :condition (and 
						...
						(over all (at ?r ?o)) 
						(over all (linked ?o ?d)) 
						(over all (allowed ?r ?d))
						(at start(>= (battery ?r)
							(/ 1 (/ (distancia ?o ?d) (velocidad ?r)))
						))
					)
         :effect
	            (and 
				   	(at end (not (at ?r ?o)))
					(at end (at ?r ?d))
					(at end(increase (total-battery-used) 
							  (/ 1 (/ (distancia ?o ?d) (velocidad ?r)))
					))
					(at end(decrease (battery ?r) 
							  (/ 1 (/ (distancia ?o ?d) (velocidad ?r)))
					))
				)
	)
  ...
  (:durative-action give-cup
		 ...
		 :condition (and 
						...
						(at start (at ?r2 ?l)) 
						(over all(at ?r2 ?l)) 
						(at start (have ?t ?r1 ?b1)) 
						(over all (have ?t ?r1 ?b1)) 
						(at start (libre ?b2 ?r2)) 
						(over all (libre ?b2 ?r2)) 
						(at start (ocupado ?b1 ?r1)) 
						(over all (ocupado ?b1 ?r1))
						(at start (>= (battery ?r1) (/ 1 (peso ?t))))
						(over all (>= (battery ?r1) (/ 1 (peso ?t))))
					)
    	 :effect
	     		(and 
					(at end (have ?t ?r2 ?b2)) 
					...
					(at end (at ?r1 ?l))
					(at end (at ?r2 ?l))
					(at end (increase (total-battery-used) (/ 1 (peso ?t))))
					(at end (decrease (battery ?r1) (/ 1 (peso ?t))))
				)
	)
  ...
  (:durative-action fill-cup
		 ...
		 :condition (and 
						...
						(at start (empty ?t))
						(over all (empty ?t))
						(at start (>= (battery ?r) (/ 1 2)))
						(over all (>= (battery ?r) (/ 1 2)))
					)
	     :effect
	     		(and 
				 	(at end (not (empty ?t)))
					...
					(at end (at ?r ?l))
					(at end (have ?t ?r ?b))
					(at end (increase (total-battery-used) (/ 1 2)))
					(at end (decrease (battery ?r) (/ 1 2)))
				)
	)
  ...
  (:durative-action serve-cup
		 ...
		 :condition (and 
						...
						(at start (full ?t)) 
						(over all (full ?t)) 
						(at start (ocupado ?b ?r)) 
						(over all (ocupado ?b ?r))
						(at start (>= (battery ?r) (/ 1 (peso ?t))))
						(over all (>= (battery ?r) (/ 1 (peso ?t))))
					)
	     :effect
	     		(and 
					...
					(at end (at ?p ?l))
					(at end (increase (total-battery-used) (/ 1 (peso ?t))))
					(at end (decrease (battery ?r) (/ 1 (peso ?t))))
				)
	)
  ...
  (:durative-action renove
		:parameters (?r - robot)
		:duration (= ?duration 2)
		:condition (at start (< (battery ?r) (umbral)))
		:effect (at end (assign (battery ?r) (+ (umbral) 2)))
	)
)
