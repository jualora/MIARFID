...
#                                                              #
################################################################
def distance(origin, destino):
    o = origin.agent.get.get_position()
    distancia = math.sqrt((destino[0]-o[0])**2 + (destino[1]-o[1])**2)
    return distancia
...
class myFleetManagerStrategy(FleetManagerStrategyBehaviour):
    ...
    async def run(self):
        ...
        if msg:
            transportes = self.get_transport_agents().values()
            #Ordenamos los transportes del mas cercano al mas lejano al usuario del mensaje
            sorted(transportes, key = lambda x: distance(x) = distance(self, msg.origin))
