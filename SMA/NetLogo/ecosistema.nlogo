turtles-own [energy] ;; para hacerle el sefuimiento a la tortuga cuando esté lista
                                     ;; para reproducirse y para morir

breed [tortugas tortuga]
breed [supertortugas supertortuga]
...
to setup-patches
  ask patches [ ifelse random 100 < green% [set pcolor green] [set pcolor blue]]
end

to setup-turtles
  create-tortugas 0    ;; uses the value of the number slider to create turtles
  ask tortugas [ setxy random-xcor random-ycor set shape "person" set color yellow]
  create-supertortugas num-agents / 2
  ask supertortugas [setxy random-xcor random-ycor set shape "person" set color orange]
end
...
to go
  ...
  eat-grass
  if cruce? [cruce]
  if offspring [reproduce]
  if death [check-death]
  if grass-growing [regrow-grass]
...
to eat-grass
  ask supertortugas [
    ...
    if pcolor = blue [
      set pcolor black
      set energy (energy + 2 * energy-from-grass)
    ]
    ifelse show-energy?
...
to regrow-grass
  ask patches [ ;; 3 out of 100 times, the patch color is set to green
    if random 100 < grow% [ifelse random 100 < green% [ set pcolor green ] [ set pcolor blue ]]
  ]
end

to cruce
  ask patches with [count tortugas-here >= 2] [ask one-of tortugas-here [hatch-supertortugas 1[set shape "person" set color orange]] ask n-of 2 tortugas-here [die]]
  ask patches with [count supertortugas-here >= 2] [ask one-of supertortugas-here [hatch-tortugas 1[set shape "person" set color yellow]] ask n-of 2 supertortugas-here [die]]
end
...
@#$#@#$#@
<experiments>
  <experiment name="experiment_ecosistema" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    ...
    <enumeratedValueSet variable="cruce?">
      <value value="true"/>
      <value value="false"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="grass-growing">
      <value value="true"/>
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="green%">
      <value value="50"/>
      <value value="75"/>
    </enumeratedValueSet>
